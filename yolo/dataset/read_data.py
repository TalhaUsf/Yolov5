...
class DataReader(object):
    ...
    def __getitem__(self, idx):
        ...
        if self.mosaic:  # mosaic need to load 4 images
            ...
            img, label = load_mosaic_image(
                idx, mosaic_border, self.img_size, self.images_dir, self.labels_ori
            )
        else:
        ...
        if self.transforms:
            img, label = self.transforms(
                img, label, mosaic=self.mosaic, augment=self.augment
            )
        img, label = resize_image(
            img, self.img_size, keep_ratio=True, label=label
        )  # resize the image
    ...
    def load_mixup_image_and_label(self, idx):
        img, label = self.load_image_and_label(idx)
        r_img, r_label = self.load_image_and_label(
            random.randint(0, len(self.images_dir) - 1)
        )
