...
class AnchorLabeler(object):
    ...
    def encode(self, labels):
        ...
        for i in range(self.num_scales):
            ...
            y_true = tf.zeros(
                [grid_size, grid_size, self.n_anchor_per_scale, 6], tf.float32
            )
            gain = tf.tensor_scatter_nd_update(
                gain, [[0], [1], [2], [3]], [grid_size] * 4
            )
            scaled_labels = (
                labels * gain
            )  # label coordinator now is the same with anchors

            if labels is not None:
                gt_wh = scaled_labels[..., 2:4]  # n_gt * 2
                if self.assign_method == "wh":
                    assert (
                        self.anchor_match_threshold > 1
                    ), "threshold is totally different for wh and iou assign"
                    matched_matrix = self.assign_criterion_wh(
                        gt_wh, anchor, self.anchor_match_threshold
                    )
                elif self.assign_method == "iou":
                    assert (
                        self.anchor_match_threshold < 1
                    ), "threshold is totally different for wh and iou assign"
                    matched_matrix = self.assign_criterion_iou(
                        gt_wh, anchor, self.anchor_match_threshold
                    )
                else:
                    raise ValueError
                ...
                assigned_anchor = tf.tile(
                    tf.reshape(
                        tf.range(self.n_anchor_per_scale), (self.n_anchor_per_scale, 1)
                    ),
                    (1, n_gt),
                )

                assigned_anchor = tf.expand_dims(
                    assigned_anchor[matched_matrix], 1
                )  # filter
                assigned_anchor = tf.cast(assigned_anchor, tf.int32)

                assigned_label = tf.tile(
                    tf.expand_dims(scaled_labels, 0), [self.n_anchor_per_scale, 1, 1]
                )
                ...
                if self.extend_offset:
                    (
                        ...
                        assigned_anchor,
                        grid_offset,
                    ) = self.enrich_pos_by_position(
                        assigned_label, assigned_anchor, gain, matched_matrix
                    )
                else:
                    grid_offset = tf.zeros_like(assigned_label[:, 0:2])

                assigned_grid = tf.cast(
                    assigned_label[..., 0:2] - grid_offset, tf.int32
                )  # n_matched * 2
                assigned_grid = tf.clip_by_value(
                    assigned_grid, clip_value_min=0, clip_value_max=grid_size - 1
                )

                # tensor: grid * grid * 3 * 6, indices（sparse index）: ~n_gt * gr * gr * 3, updates: ~n_gt * 6
                assigned_indices = tf.concat(
                    [assigned_grid[:, 1:2], assigned_grid[:, 0:1], assigned_anchor],
                    axis=1,
                )
                ...
                y_true = tf.tensor_scatter_nd_update(
                    y_true, assigned_indices, assigned_updates
                )
            y_anchor_encode.append(y_true)
        return tuple(
            y_anchor_encode
        )  # add a tuple is important here, otherwise raise an error
    ...
    def enrich_pos_by_position(
        self, assigned_label, assigned_anchor, gain, matched_matrix, rect_style="rect4"
    ):
        ...
        if rect_style == "rect2":
            ...
            matched = (assigned_xy % 1.0 > (1 - g)) & (
                assigned_xy < tf.expand_dims(gain[0:2], 0) - 1.0
            )
