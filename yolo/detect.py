...
from dataset.image_utils import resize_back, resize_image
from model.post_process import batch_non_max_suppression
from tools.vis_data import draw_box


def image_demo(
    img, model, img_size=640, class_names=None, conf_threshold=0.4, iou_threshold=0.3
):
    ...
    pred_bbox = [
        tf.reshape(x, (tf.shape(x)[0], -1, tf.shape(x)[-1])) for x in pred_bbox
    ]
    pred_bbox = tf.concat(pred_bbox, axis=1)  # batch_size * -1 * (num_class + 5)

    bboxes = batch_non_max_suppression(
        pred_bbox, conf_threshold=conf_threshold, iou_threshold=iou_threshold
    )
    bboxes = bboxes[0].numpy()  # batch is 1 for detect

    bboxes = resize_back(
        bboxes, target_sizes=img_size, original_shape=original_shape
    )  # adjust box to original size
    if bboxes.any():
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--img_dir",
        type=str,
        default="../data/voc2012/VOCdevkit/VOC2012/JPEGImages/2008_001210.jpg",
    ...
    parser.add_argument(
        "--model_dir", type=str, default="../weights/yolov5", help="saved pb model dir"
    )
    parser.add_argument("--img_size", type=int, default=640, help="image target size")
    parser.add_argument(
        "--conf_threshold", type=float, default=0.4, help="filter confidence threshold"
    )
    parser.add_argument(
        "--iou_threshold", type=float, default=0.3, help="nms iou threshold"
    )
