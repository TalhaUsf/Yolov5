...
class TestDataReader(DataReader):
    def __init__(
        ...
        super(TestDataReader, self).__init__(
            annotations_dir, image_target_size, transform, mosaic, augment
        )
...
class Evaluator(object):
    ...
    def generate(self, test_dataset, results_dir):
        ...
        for image_id, image_original_shape, img, labels in test_dataset:
            ...
            predictions = [
                tf.reshape(x, (tf.shape(x)[0], -1, tf.shape(x)[-1]))
                for x in predictions
            ]
            # batch_size * -1 * (num_class + 5)
            predictions = tf.concat(predictions, axis=1)
            # a list with n_image length, each element is n_pred * 6
            preds = batch_non_max_suppression(
                predictions, conf_threshold=0.4, iou_threshold=0.35
            )
            ...
            for i, label in enumerate(labels):  # iter for image, gt
                with open(
                    results_dir + "/ground-truth/{}.txt".format(image_id[i]), "w"
                ) as f:
                    for l in label:
                        ...
                        bbox_message = (
                            " ".join([class_name, xmin, ymin, xmax, ymax]) + "\n"
                        )
                        f.write(bbox_message)

            for i, (pred, original_shape) in enumerate(
                zip(preds, image_original_shape)
            ):  # iter for image, pred
                if not pred.any():
                    with open(
                        results_dir + "/predicted/{}.txt".format(image_id[i]), "w"
                    ) as f:
                        f.write("")
                    break
