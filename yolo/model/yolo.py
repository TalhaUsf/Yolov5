...
class Yolo(object):
    ...
    def parse_model(self, yaml_dict):
        ...
        depth_multiple, width_multiple = (
            yaml_dict["depth_multiple"],
            yaml_dict["width_multiple"],
        )
        ...
        for i, (f, number, module, args) in enumerate(
            yaml_dict["backbone"] + yaml_dict["head"]
        ):
            ...
            for j, arg in enumerate(args):
                try:
                    args[j] = (
                        eval(arg) if isinstance(arg, str) else arg
                    )  # eval strings, like Detect(nc, anchors)
                except:
                    pass

            number = (
                max(round(number * depth_multiple), 1) if number > 1 else number
            )  # control the model scale
            ...
            modules = (
                tf.keras.Sequential(*[module(*args) for _ in range(number)])
                if number > 1
                else module(*args)
            )
...
class Detect(Layer):
    def __init__(self, num_classes, anchors=()):
        ...
        self.anchors = tf.cast(
            tf.reshape(anchors, [self.num_anchors, -1, 2]), tf.float32
        )
        self.modules = [
            Conv2D(self.output_dims * self.num_anchors, 1, use_bias=False)
    ...
    def call(self, x, training=True):
        ...
        for i in range(self.num_scale):  # number of scale layer, default=3
            ...
            grid_xy = tf.cast(
                tf.expand_dims(tf.stack(grid_xy, axis=-1), axis=2), tf.float32
            )

            y_norm = tf.sigmoid(y)  # sigmoid for all dims
            xy, wh, conf, classes = tf.split(
                y_norm, (2, 2, 1, self.num_classes), axis=-1
            )
