...
# # --------------------------------------------------------------------------
# #                              for high level .fit api use following                        
# # --------------------------------------------------------------------------
class YoloLoss_fit_api(object):
    def __init__(
        self, anchors, ignore_iou_threshold, num_classes, img_size, label_smoothing=0
    ):
        ...
        self.bce_conf = tf.keras.losses.BinaryCrossentropy(
    ...
    def __call__(self, y_true, y_pred):
        Console().print("yolo loss function is called")
        Console().print("⚡ y_true shape is ", y_true.shape)
        Console().print("⚡ y_pred shape is ", y_pred.shape)
        ...
        for i, (pred, true) in enumerate(zip(y_pred, y_true)):
            ...
            if tf.shape(true_class)[-1] == 1 and self.num_classes > 1:
                true_class = tf.squeeze(
                    tf.one_hot(
                        tf.cast(true_class, tf.dtypes.int32),
            ...
            box_scale = 2 - 1.0 * true_box[..., 2] * true_box[..., 3] / (
                self.img_size**2
            )
            obj_mask = tf.squeeze(
                true_obj, -1
            )  # obj or noobj, batch_size * grid * grid * anchors_per_grid
            ...
            conf_loss = conf_focal * (
                obj_mask * conf_loss + background_mask * conf_loss
            )  # batch * grid * grid * 3
            ...
            class_loss_all += (
                class_loss * self.num_classes * balance[i]
...
# # --------------------------------------------------------------------------
# #                       for custom training loop , do this                        
# # --------------------------------------------------------------------------
...
class YoloLoss(object):
    ...
    def __call__(self, y_true, y_pred):
        iou_loss_all = obj_loss_all = class_loss_all = 0
        balance = (
            [1.0, 1.0, 1.0] if len(y_pred) == 3 else [4.0, 1.0, 0.4, 0.1]
        )  # P3-5 or P3-6
...
def bbox_iou(
    bbox1, bbox2, xyxy=False, giou=False, diou=False, ciou=False, epsilon=1e-9
):
