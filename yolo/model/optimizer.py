...
class Step(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, global_step):
        ...
        assert warmup_steps < self.total_steps, "warmup {}, total {}".format(
            warmup_steps, self.total_steps
        )

        linear_warmup = warmup_lr + tf.cast(global_step, tf.float32) / warmup_steps * (
            init_lr - warmup_lr
        )
        ...
        for next_learning_rate, start_step in zip(lr_levels, lr_steps):
            learning_rate = tf.where(
                global_step >= start_step, next_learning_rate, learning_rate
            )
...
class Cosine(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, global_step):
        init_lr = self.params["init_learning_rate"]
        warmup_lr = (
            self.params["warmup_learning_rate"]
            if "warmup_learning_rate" in self.params
            else 0.0
        )
        warmup_steps = self.params["warmup_steps"]
        assert warmup_steps < self.total_steps, "warmup {}, total {}".format(
